...
// Scene_Prologue: a book-style story intro shown before the first map,
// mirroring the opening cutscene / narration crawl common to JRPG openings.
export class Scene_Prologue implements Scene {
  private pageIndex = 0;
  private charProgress = 0;
  private time = 0;

  private get pages(): string[] {
    return DataManager.story().prologue;
  }
  ...
  draw(ctx: CanvasRenderingContext2D) {
    drawBackdrop(ctx, "#0c0f1e", "#050509");
    drawStarfield(ctx, this.time, 90);

    drawText(ctx, DataManager.story().worldName, ctx.canvas.width / 2, 40, Palette.gold, "bold 26px sans-serif", "center");
    ...
    // introduce the three heroes waiting at the bottom of the frame
    GameParty.members.forEach((m, i) => {
      ...
      drawText(ctx, m.className, x + 60, y + 26, Palette.textDim, "12px sans-serif");
    });
  }
}
